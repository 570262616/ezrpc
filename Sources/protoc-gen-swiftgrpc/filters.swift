...
class GRPCFilterExtension : Extension {
    override init() {
        ...
        ext.registerFilter("serviceName") { (value: Any?, arguments: [Any?]) in
            if let value = value as? SwiftProtobufPluginLibrary.ServiceDescriptor {
                return value.name
            }
            throw invalidArgumentType(filter:"serviceName", required:"ServiceDescriptor", received:value)
        }
